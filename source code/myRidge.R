...
ridge.proj <- function(x, y,index,family = "gaussian"){
  standardize = TRUE
  betainit = "cv lasso"
  sigma = NULL
  n <- nrow(x)
  ...
  scaleb <- 1 / scale.vec
  se     <- 1 / scaleb
  upper= hat.betacorr[index] / sds[index] + 1.96 * se[index] / sds[index]
  lower= hat.betacorr[index] / sds[index] - 1.96 * se[index] / sds[index]
  
  rs <- list(
    est_ddl = hat.betacorr[index] / sds[index],
    se = se[index] / sds[index],
    pval = res.pval[index],
    CI=c(lower,upper)
  )
  return(rs)
