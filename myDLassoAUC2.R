...
### DLassoAUC
myDLassoAUC0222_2 <- function(X, Y, index, K.factors, rho, rhop, sig_level = 0.05){
  ...
  lam_max = 40*max(abs(t(XX)%*%Y)/(n*p))
  # the_lambdas = seq(lam_min,lam_max,(lam_max - lam_min)/10)
  the_lambdas = seq(lam_max,lam_min,(lam_min - lam_max)/20)
  ...
  for (i in seq(length(index))) {
    ...
    ### sigmahat
    {
      ...
      the_lambdas = seq(lam_max,lam_min,(lam_min - lam_max)/20)
      fit <- glmnet::cv.glmnet(x = XX, y = Y,lambda=the_lambdas, penalty.factor = c(rep(1,ncol(XX) - ncol(U)),rep(0,ncol(U))))
